...
public class TintLayout extends FrameLayout {
    ...
    public TintLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray = context.obtainStyledAttributes(attrs, com.zhan.budget.R.styleable.TintLayout);

        try {
            final double tempAngle = typedArray.getFloat(com.zhan.budget.R.styleable.TintLayout_angle, 45f);
            if (tempAngle >= 0 && tempAngle <= 360) {
                this.angle = tempAngle;
            } else {
                throw new IllegalArgumentException("Need to be between 0 and 360");
            }

            final int colorsId = typedArray.getResourceId(com.zhan.budget.R.styleable.TintLayout_colors, 0);
            if (colorsId != 0) {
                setColors(getResources().getIntArray(colorsId));
            } else {
                setColors(getResources().getIntArray(R.array.tint_shadow));
            }
        } finally {
            typedArray.recycle();
        }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (getChildCount() == 1) {
            ...
            throw new IllegalArgumentException("Cannot have more than 1 child in tintLayout");
        }
    }
